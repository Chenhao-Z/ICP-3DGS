...
class CFGaussianModel:
    ...
    def add_gaussian(self, pcd: BasicPointCloud):

        new_points = torch.tensor(np.asarray(pcd.points)).float().cuda()
        if self.view_dependent:
            new_colors = RGB2SH(torch.tensor(np.asarray(pcd.colors)).float().cuda())
        else:
            new_colors = torch.tensor(np.asarray(pcd.colors)).float().cuda()


        new_features = torch.zeros(
            (new_colors.shape[0], 3, (self.max_sh_degree + 1) ** 2)).float().cuda()
        new_features[:, :3, 0] = new_colors[:, :3]
        ...
        new_rots = torch.zeros((new_points.shape[0], 4), device="cuda")
        new_rots[:, 0] = 1

        new_opacity = inverse_sigmoid(
            0.1 * torch.ones((new_points.shape[0], 1), dtype=torch.float, device="cuda"))

        self.densification_postfix(
            new_points, new_features[:, :, 0:1].transpose(
            1, 2).contiguous(), new_features[:, :, 1:].transpose(1, 2).contiguous(), new_opacity, new_scales, new_rots)
