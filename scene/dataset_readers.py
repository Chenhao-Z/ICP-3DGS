...
from scene.colmap_loader import read_extrinsics_text, read_intrinsics_text, qvec2rotmat, \
    read_extrinsics_binary, read_intrinsics_binary, read_points3D_binary, read_points3D_text,read_json
...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder,masks_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        if intr.model == "SIMPLE_PINHOLE" or intr.model == "SIMPLE_RADIAL":
            ...
            intr_mat = np.array([[focal_length_x, 0, width / 2], [0, focal_length_x, height / 2], [0, 0, 1]])
        elif intr.model == "PINHOLE":
            focal_length_x = intr.params[0]
            focal_length_y = intr.params[1]
            FovY = focal2fov(focal_length_y, height)
            FovX = focal2fov(focal_length_x, width)
            #intr_mat = np.array([[focal_length_x, 0, width/2], [0, focal_length_y, height/2], [0, 0, 1]])
            intr_mat = np.array([[focal_length_x, 0,intr.params[2] ], [0, focal_length_y, intr.params[3]], [0, 0, 1]])
        else:
        ...
        if os.path.exists(masks_folder):
            ...
            print(f'{masks_folder} does not exist')
            mask = None
...
def readColmapSceneInfo(path, images, eval, llffhold=8):
    try:
        ...
        try:
            ...
            print("Try loading json files: {e}")
            cameras_json = os.path.join(path, "sparse/0", "metas")
            cam_extrinsics ,cam_intrinsics = read_json(cameras_json)

    reading_dir = "images" if images == None else images
    masks_folder = os.path.join(path,'masks')
    cam_infos_unsorted = readColmapCameras(
        cam_extrinsics=cam_extrinsics, cam_intrinsics=cam_intrinsics, images_folder=os.path.join(path, reading_dir),masks_folder = masks_folder)
    #print('the intrinsic is {}'.format(cam_intrinsics[1].params))
    ...
    scene_info = SceneInfo(point_cloud=pcd,
                           ...
                           ply_path=ply_path)
    #print(scene_info.getTestCameras()[0].intrinsics)
    return scene_info
