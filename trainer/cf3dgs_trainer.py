...
class CFGaussianTrainer(GaussianTrainer):
    ...
    def apply_mask_and_transform(self,pcd,mask,transform = None):
        if isinstance(pcd, BasicPointCloud):
            pcd_data = o3d.geometry.PointCloud()
            pcd_data.points = o3d.utility.Vector3dVector(pcd.points[mask])
            pcd_data.colors = o3d.utility.Vector3dVector(pcd.colors[mask])
            pcd_data.estimate_normals()
            if transform is not None:
                pcd_data.transform(transform)
            ...
            pcd_masked = BasicPointCloud(points, colors, normals)
            #print(len(pcd_data.points))
            return pcd_masked
        else:
            raise TypeError(" Must be BasicPointCloud Type")

    def pcd_transform(self,pose,pcd):
        cam_to_world = np.linalg.inv(pose.cpu().detach().numpy())
        t = cam_to_world[:3,3]
        prj = np.dot(pcd.points,t)
        mask = (prj > np.mean(prj)) & (pcd.points[:, 2] != max(pcd.points[:,2]))
        pcd_transformed = self.apply_mask_and_transform(pcd, mask, cam_to_world)
        return pcd_transformed



    def select_voxel_pcd(self, pc2, voxel_size=0.1, r = 0.5,ratio_threshold = 1):
        pc1 = self.gs_render.gaussians._xyz.detach()
        pc2 = torch.tensor(pc2, dtype=torch.float32, device="cuda")

        min_bounds = torch.min(pc2, dim=0)[0]
        max_bounds = torch.max(pc2, dim=0)[0]

        in_bounds_mask = (
                (pc1 >= min_bounds) & (pc1 <= max_bounds)
        ).all(dim=1)
        pc1_in_bounds = pc1[in_bounds_mask]

        voxel_coords1 = torch.floor(pc1_in_bounds / voxel_size).int()
        voxel_coords2 = torch.floor(pc2 / voxel_size).int()

        def get_voxel_counts(voxel_coords):
            unique_voxels, counts = torch.unique(voxel_coords, return_counts=True, dim=0)
            return unique_voxels, counts

        voxels1, counts1 = get_voxel_counts(voxel_coords1)
        voxels2, counts2 = get_voxel_counts(voxel_coords2)

        voxel_dict1 = torch.zeros_like(counts2, dtype=torch.float32, device="cuda")
        for i, v in enumerate(voxels2):
            matches = (voxels1 == v).all(dim=1)
            if matches.any():
                voxel_dict1[i] = counts1[matches].item()

        voxel_ratios = counts2 / (voxel_dict1 + 1e-6)
        valid_ratios_mask = voxel_ratios >= ratio_threshold
        filtered_voxels = voxels2[valid_ratios_mask]
        filtered_ratios = voxel_ratios[valid_ratios_mask]
        sorted_indices = torch.argsort(filtered_ratios, descending=True)
        top_n = int(r * len(sorted_indices))
        top_voxels = filtered_voxels[sorted_indices[:top_n]]

        top_voxel_mask = (voxel_coords2[:, None, :] == top_voxels[None, :, :]).all(dim=2).any(dim=1)

        print(top_voxel_mask.sum().item())
        return top_voxel_mask.cpu().numpy()
    ...
    def add_view_v2(self, view_idx, view_idx_prev, reverse=False):
        ...
        cam_info, pcd_cur, viewpoint_cam = self.prepare_data(view_idx,
                                                         orthogonal=True,
                                                         down_sample=True)

        trans_cur_pcd = self.pcd_transform(pose, pcd_cur)
        selected_mask = self.select_voxel_pcd(trans_cur_pcd.points)
        selected_pcd = self.apply_mask_and_transform(trans_cur_pcd, selected_mask)
        self.gs_render.gaussians.add_gaussian(selected_pcd)
    ...
    def general_views(self,start_frame, last_frame):
        self.loss_func.depth_loss_type = "invariant"
        pipe = copy(self.pipe_cfg)
        optim_opt = copy(self.optim_cfg)
        #print("train_num", train_num)
        num_iterations = (last_frame - start_frame) *50
        progress_bar = tqdm(range(num_iterations), desc="Training progress")

        for iteration in range(1, num_iterations + 1):
            fidx = randint(start_frame, last_frame-1)
    ...
    def train_from_progressive(self, ):
        ...
        for epoch in range(num_eppch):
            ...
            self.general_views(start_frame=0, last_frame=length - 1)
            # self.general_views(0,length//20)
            # self.general_views(length-length//20,length-1)
            ...
            with torch.no_grad():
                ...
                for idx in self.i_test:
                    pose = self.gs_render.gaussians.get_RT(idx)
                    pose_dict["poses_pred_test"].append(pose.detach().cpu())

            pose_dict["poses_pred"] = torch.stack(pose_dict["poses_pred"])
            pose_dict["poses_pred_test"] = torch.stack(pose_dict["poses_pred_test"])
            pose_dict["poses_gt"] = torch.stack(poses_gt)
