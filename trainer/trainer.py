...
class GaussianTrainer(object):
    ...
    def __init__(self, data_root, model_cfg, pipe_cfg, optim_cfg):
        ...
        self.GT_pose = model_cfg.GT_pose
        self.BG_free = model_cfg.BG_free
        self.non_random = model_cfg.non_random
        self.ply_initial = model_cfg.ply_initial
        ...
        self.save_ply = model_cfg.save_ply
        self.print_pose = model_cfg.print_pose
        self.rel_GT_Pose = model_cfg.rel_GT_Pose
        self.use_sky_mask = model_cfg.use_sky_mask
    ...
    def setup_depth_predictor(self,):
        ...
        elif self.depth_model_type == "Metric3Dv2":
            model_Metric3DV2 = torch.hub.load('yvanyin/metric3d', 'metric3d_vit_giant2', pretrain=True)
            Metric3DV2 = model_Metric3DV2.to(device)
            self.depth_model = Metric3DV2
    ...
    def predict_depth(self, img):
        if self.depth_model_type == "zoe":
            ...
            depth = torch.nn.functional.interpolate(pred_depth[None, None, :, :], img.shape[:2],mode='bilinear').squeeze()
    ...
    def setup_dataset(self):
        if self.model_cfg.data_type == "co3d":
            ...
            #print(test_views[0].image_name)
            #print(test_views[0].R)
            #print(test_views[0].T)
            train_views = scene.getTrainCameras().copy()
            #print(train_views[0].intrinsics)
    ...
    def prepare_data_from_viewpoint(self, idx, down_sample=False,
                                    orthogonal=False, pose=None,
        ...
        if getattr(viewpoint_camera, 'original_image', None) is not None:
            ...
            mask_torch = viewpoint_camera.mask
        else:
            #print('R will keep the same')
            ...
            mask_pil = Image.open(viewpoint_camera.mask_path)
            mask_np = np.asarray(mask_pil)
            mask_torch =  torch.from_numpy(mask_np!=0)
        ...
        if self.use_sky_mask:
            depth_mask = mask_torch.to(depth_tensor.device)
        else:
            if self.BG_free:
                depth_mask = depth_tensor != torch.max(depth_tensor)
            else:
        ...
        points = pts[0].permute(1, 2, 0)[depth_mask].cpu().numpy().reshape(-1, 3)
        viewpoint_camera = Camera(idx, R, t, FoVx, FoVy, image_torch,
                                  gt_alpha_mask=mask, image_name=image_name,
                                  intrinsics=intrinsics,
                                  uid=idx, is_co3d=True, mask = mask_torch)
        pcd_data = o3d.geometry.PointCloud()
        pcd_data.points = o3d.utility.Vector3dVector(points)

        depth_mask = depth_mask.cpu()
        pcd_data.colors = o3d.utility.Vector3dVector(image_np[depth_mask].reshape(-1, 3))
        pcd_data.estimate_normals()

        if down_sample:
            pcd_data = pcd_data.voxel_down_sample(voxel_size=0.01)
        if self.save_ply:
            o3d.io.write_point_cloud('./depth_lifted_pcd/ply_{}.ply'.format(image_name.split('.')[0]), pcd_data)
        colors = np.asarray(pcd_data.colors, dtype=np.float32)
        points = np.asarray(pcd_data.points, dtype=np.float32)
        normals = np.asarray(pcd_data.normals, dtype=np.float32)
        pcd = BasicPointCloud(points, colors, normals)
    ...
    def prepare_data(self, idx, down_sample=True,
                     orthogonal=False, learn_pose=False,
                     pose=None, load_depth=True,
                     load_gt=False):
    ...
    def load_viewpoint_cam(self, idx, pose=None, load_depth=False):
        if self.data_type == "co3d":
            ...
            if getattr(viewpoint_camera, 'original_image', None) is not None:
                ...
                sky_mask = viewpoint_camera.mask
            else:
                R = viewpoint_camera.R
                t = viewpoint_camera.T
                FoVx = viewpoint_camera.FovX
            ...
            viewpoint_camera = Camera(idx, R, t, FoVx, FoVy, original_image,
                                      gt_alpha_mask=None, image_name=image_name,
                                      intrinsics=intrinsics,
                                      uid=idx, is_co3d=True,mask = sky_mask)
    ...
    def load_RT(self, idx, pose=None, load_depth=False):
        if self.data_type == "co3d":
            image_name = self.data[idx]["image"]["path"]
            image_path = os.path.join(self.data_root, image_name)
            R, t, FoVx, FoVy, intr_mat = self.load_camera(self.data[idx])
            ...
            viewpoint_camera = copy(self.data[idx])
            if getattr(viewpoint_camera, 'original_image', None) is not None:
                R_ = viewpoint_camera.R.transpose()
    ...
    def train(self, pipe, optim_opt, viewpoint_cam, colors_precomp=None):
        ...
        progress_bar = tqdm(range(optim_opt.iterations),ncols = 130,
